...
- (void)setInternalSKKEnabled:(BOOL)state {
  [_webView evaluateJavaScript:term_setInternalSKK(state) completionHandler:nil];
}

- (void)noConvertComposition {
  [_webView evaluateJavaScript:term_noConvertComposition() completionHandler:nil];
}
      ...
      didReceiveScriptMessage:(WKScriptMessage *)message
{
  ...
  if ([operation isEqualToString:@"selectionchange"]) {
    ...
    [_gestureInteraction scrollPrimaryToBottomForLocalInput];
    ...
    NSLog(@"%@", [NSString stringWithFormat:@"BlinkJS %@", data[@"message"] ?: @""]);
    if (data[@"extra"]) {
      NSLog(@"%@", [NSString stringWithFormat:@"BlinkJS extra=%@", data[@"extra"]]);
    }
